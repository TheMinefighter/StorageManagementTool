...
namespace UniversalCommandlineInterface.Interpreters {
   public class ActionInterpreter : BaseInterpreter {
      ...
      internal override bool Interpret(bool printErrors = true) {
         ...
         if (GetValues(search, invokationArguments, out object value)) {
            return true;
         }
      ...
      private bool GetValues(string search, IDictionary<CmdParameterAttribute, object> invokationArguments, out object value) {
         value = null;
         if (IsParameterDeclaration(out CmdParameterAttribute found)) {
            ...
            if (IsAlias(found, out object aliasValue)) {
               ...
               else if (parameterType == typeof(IEnumerable<>).MakeGenericType(realType)) {
               }
