        ...
        private void SearchTi_OnSelected(object sender, RoutedEventArgs e)
        {
            if (!_searchTiOpened)
            {
                if (!Session.Singleton.IsAdmin) {
                    Wrapper.RestartProgram(true, new []{"-UI","-UIProperties", Newtonsoft.Json.JsonConvert.SerializeObject(new Dictionary<string,object> {{nameof(BaseTc)+','+nameof(TabControl.SelectedIndex),3}})},true);
                }
