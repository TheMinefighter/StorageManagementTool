...
namespace StorageManagementCore.WPFGUI.ValueConverters {
	public class ShellFolderFilterConverter : IMultiValueConverter {
		public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture) {
			...
			if (values.Length != 2) {
				throw new ArgumentException("values must contain exactly two elements", nameof(values));
			}
		...
		public object[] ConvertBack(object value, Type[] targetTypes, object parameter, CultureInfo culture) {
			...
			if (targetTypes[0] != typeof(IReadOnlyCollection<ShellFolder>)) {
				throw new ArgumentException("The first targeted type must be an IReadOnlyCollection<ShellFolder>",
					nameof(targetTypes));
			}
